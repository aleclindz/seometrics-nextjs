...
// ==========================================
// DYNAMIC ROBOTS.TXT SERVING - AUTOMATION
// ==========================================

function initializeRobotsServing() {
    try {
        console.log('[SEO-METRICS] Initializing dynamic robots.txt serving...');
        
        // Check if we're on the robots.txt path
        if (window.location.pathname === '/robots.txt') {
            console.log('[SEO-METRICS] Detected robots.txt request, serving dynamic robots.txt');
            serveDynamicRobots();
            return;
        }
        
        // Set up service worker for intercepting robots.txt requests
        if ('serviceWorker' in navigator) {
            setupRobotsServiceWorker();
        } else {
            // Fallback: Use history API to intercept navigation
            setupRobotsInterception();
        }
        
    } catch (error) {
        console.error('[SEO-METRICS] Error initializing robots.txt serving:', error);
    }
}

function serveDynamicRobots() {
    try {
        console.log('[SEO-METRICS] Generating dynamic robots.txt for:', window.location.hostname);
        
        // Generate robots.txt content
        generateAndServeRobots();
        
    } catch (error) {
        console.error('[SEO-METRICS] Error serving dynamic robots.txt:', error);
    }
}

async function generateAndServeRobots() {
    try {
        // Get robots.txt data from API or generate locally
        let robotsContent = await fetchRobotsFromAPI();
        
        if (!robotsContent) {
            // Fallback: Generate basic robots.txt locally
            robotsContent = generateBasicRobotsContent();
        }
        
        if (robotsContent) {
            // Replace page content with robots.txt content
            document.open();
            ...
            console.log('[SEO-METRICS] Robots.txt served successfully');
        }
        
    } catch (error) {
        console.error('[SEO-METRICS] Error generating robots.txt:', error);
    }
}

async function fetchRobotsFromAPI() {
    try {
        const response = await fetch('https://seoagent.com/api/technical-seo/robots-generate', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
        ...
        if (response.ok) {
            ...
            return data.robotsContent;
        }
        
    } catch (error) {
        console.log('[SEO-METRICS] API robots fetch failed, using local generation:', error);
    }
    
    return null;
}

function generateBasicRobotsContent() {
...
# Common directories to block (if they exist)
Disallow: /admin/
Disallow: /wp-admin/
...
# Block API endpoints (optional - comment out if you want them indexed)
# Disallow: /api/
# Disallow: /rest/
# Disallow: /graphql/
...
# Generated by SEOAgent.js on ${new Date().toISOString().split('T')[0]}
`;
}

function setupRobotsServiceWorker() {
    try {
        // Register service worker for robots.txt interception
        const swCode = `
self.addEventListener('fetch', function(event) {
    if (event.request.url.endsWith('/robots.txt')) {
        console.log('[SEO-SW] Intercepting robots.txt request:', event.request.url);
        
        event.respondWith(
            fetch('https://seoagent.com/api/technical-seo/robots-generate', {
                method: 'POST',
            ...
            .then(response => {
                if (response.ok) {
                    return response.json().then(data => {
                        if (data.robotsContent) {
                            return new Response(data.robotsContent, {
                ...
                const basicRobots = generateBasicRobots(new URL(event.request.url).hostname);
...
function generateBasicRobots(domain) {
...
\`;
        ...
        navigator.serviceWorker.register(swUrl)
            .then(() => {
                console.log('[SEO-METRICS] Robots.txt service worker registered');
            })
            .catch(error => {
                console.error('[SEO-METRICS] Service worker registration failed:', error);
                setupRobotsInterception();
            });
            
    } catch (error) {
        console.error('[SEO-METRICS] Error setting up robots service worker:', error);
        setupRobotsInterception();
    }
}

function setupRobotsInterception() {
    try {
        // Fallback: Use History API interception
        const originalPushState = history.pushState;
        const originalReplaceState = history.replaceState;
        ...
        window.addEventListener('popstate', checkForRobotsRoute);
        
        console.log('[SEO-METRICS] Robots.txt fallback interception setup complete');
        
    } catch (error) {
        console.error('[SEO-METRICS] Error setting up robots interception:', error);
    }
}

function checkForRobotsRoute() {
    if (window.location.pathname === '/robots.txt') {
        serveDynamicRobots();
    }
}
