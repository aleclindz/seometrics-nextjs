...
interface WorkflowDependency {
  type: 'integration' | 'permission' | 'data';
  requirement: string;
  description: string;
  optional: boolean;
}
...
interface WorkflowExecutionPlan {
  ...
  executionOrder: string[][]; // Batches of parallel actions
...
interface WorkflowBlockedAction {
  actionId: string;
  reason: string;
...
export class WorkflowEngine {
   ...
   */
  static suggestWorkflow(ideaTitle: string, ideaHypothesis?: string, evidence?: any): WorkflowTemplate | null {
    ...
    const scores = this.WORKFLOW_TEMPLATES.map(template => {
      ...
      if (evidence) {
        if (evidence.site_age === 'new' && template.category === 'setup') score += 2;
        if (evidence.has_technical_issues && template.category === 'technical') score += 2;
        if (evidence.content_performance === 'poor' && template.category === 'content') score += 2;
      }
  ...
  static async createExecutionPlan(
    ...
    const blockedActions: WorkflowBlockedAction[] = [];
    const readyActions: string[] = [];
    
    workflowTemplate.actions.forEach(action => {
      const missingForAction = missingDependencies.filter(dep => 
        this.actionRequiresDependency(action, dep.requirement)
      );
      ...
      if (missingForAction.length > 0) {
        ...
        const workflowDepsReady = action.dependsOn?.every(depId => 
          readyActions.includes(depId) || executionOrder[0].includes(depId)
        ) ?? true;
   ...
   */
  static async executeWorkflowPlan(
    plan: WorkflowExecutionPlan,
    userToken: string,
    siteUrl: string
  ): Promise<{ actionIds: string[], message: string }> {
   ...
   */
  static getWorkflowTemplates(category?: string, searchTerm?: string): WorkflowTemplate[] {
  ...
  private static async isDependencySatisfied(
  ...
  private static async checkIntegrationDependency(requirement: string, userToken: string, siteUrl: string): Promise<boolean> {
    switch (requirement) {
  ...
  private static async checkPermissionDependency(requirement: string, userToken: string, siteUrl: string): Promise<boolean> {
    switch (requirement) {
  ...
  private static async checkDataDependency(requirement: string, userToken: string, siteUrl: string): Promise<boolean> {
    switch (requirement) {
  ...
  private static buildExecutionOrder(actions: WorkflowAction[]): string[][] {
  ...
  private static actionRequiresDependency(action: WorkflowAction, requirement: string): boolean {
    // Simple mapping - in a real system this would be more sophisticated
    const actionDependencyMap: Record<string, string[]> = {
  ...
  private static calculatePriorityScore(action: WorkflowAction, workflowRisk: string): number {
    ...
    if (workflowRisk === 'high') score += 10;
    else if (workflowRisk === 'medium') score += 5;
    
    // Shorter duration gets slight priority boost
    if (action.estimatedDuration < 10) score += 5;
