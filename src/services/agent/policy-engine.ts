...
interface BlastRadius {
  scope: 'single_page' | 'section' | 'site_wide' | 'cross_site';
...
export class PolicyEngine {
  ...
  private static readonly DEFAULT_POLICIES: Record<string, Partial<AgentPolicy>> = {
    'technical_seo_crawl': {
  ...
  private static readonly SITE_POLICIES: Record<string, Partial<AgentPolicy>> = {};
  ...
  static async validatePolicy(
    ...
    requestedPolicy: Partial<AgentPolicy>
  ...
  ): { withinLimits: boolean; shouldStop: boolean; reason?: string } {
    // Check page limit
   ...
   */
  static getNewSitePolicy(): AgentPolicy {
    return {
      environment: 'DRY_RUN',
      maxPages: 5,
      maxPatches: 10,
  ...
  private static getBasePolicyForAction(actionType: string): AgentPolicy {
  ...
  private static async getSitePolicyOverrides(
    userToken: string,
    siteUrl: string
  ...
  private static async performSafetyValidation(
    context: ActionContext,
    policy: AgentPolicy
  ): Promise<PolicyValidationResult> {
  ...
  private static calculateRiskLevel(
    ...
    if (this.isHighRiskAction(context.actionType)) riskScore += 3;
    else if (this.isMediumRiskAction(context.actionType)) riskScore += 2;
    else riskScore += 1;

    // Environment risk
    if (policy.environment === 'PRODUCTION') riskScore += 2;
    else if (policy.environment === 'STAGING') riskScore += 1;

    // Blast radius risk
    if (policy.blastRadius.maxAffectedPages > 50) riskScore += 3;
    else if (policy.blastRadius.maxAffectedPages > 10) riskScore += 2;
    else riskScore += 1;

    // Rollback capability reduces risk
    if (policy.blastRadius.rollbackRequired) riskScore -= 1;
  ...
  private static async checkSubscriptionLimits(
    userToken: string,
    actionType: string
  ): Promise<boolean> {
    try {
  ...
  private static isHighRiskAction(actionType: string): boolean {
    return [
      'schema_injection',
      'technical_seo_fix',
      'cms_publishing',
  ...
  private static isMediumRiskAction(actionType: string): boolean {
    return [
