...
export type ArticleType = 'how_to' | 'listicle' | 'guide' | 'faq' | 'comparison' | 'evergreen' | 'blog';
...
export class ArticleTemplatesService {
  private templates: Record<ArticleType, ArticleTemplate>;
   ...
   */
  getTemplate(type: ArticleType): ArticleTemplate {
   ...
   */
  getAvailableTypes(): ArticleType[] {
    return Object.keys(this.templates) as ArticleType[];
  }

  /**
   * Calculate word target based on length and type
   */
  calculateWordTarget(contentLength: string, articleType: ArticleType): number {
    const baseTargets = {
      short: 800,
      medium: 1100,
      long: 1600
    ...
    const baseTarget = baseTargets[contentLength as keyof typeof baseTargets] || baseTargets.medium;
   ...
   */
  private getHowToStructure(): string {
    return `
...
`.trim();
   ...
   */
  private getListicleStructure(): string {
    return `
...
`.trim();
   ...
   */
  private getGuideStructure(): string {
    return `
...
`.trim();
   ...
   */
  private getFaqStructure(): string {
    return `
...
`.trim();
   ...
   */
  private getComparisonStructure(): string {
    return `
...
`.trim();
   ...
   */
  private getEvergreenStructure(): string {
    return `
...
`.trim();
   ...
   */
  getCitationRules(articleType: ArticleType, includeCitations: boolean, referenceStyle: 'link' | 'apa'): string {
    if (!includeCitations) {
      return 'Do not include references; avoid unverifiable claims and focus on general knowledge.';
    }

    const typeSpecificRules: Record<ArticleType, string> = {
