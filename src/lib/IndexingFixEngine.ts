...
export interface IndexingFixRequest {
  userToken: string;
  siteUrl: string;
  actionItemId?: string;
  problemAnalysis: any; // From IndexingIssueAnalyzer
}

export class IndexingFixEngine {
  
  /**
   * Main fix orchestrator - routes to specific fix methods
   */
  static async fixIndexingIssues(request: IndexingFixRequest): Promise<FixResult> {
   ...
   */
  private static async fixRobotsTxtBlocking(userToken: string, siteUrl: string, problems: any[]): Promise<FixResult> {
   ...
   */
  private static async fixAccessDeniedIssues(userToken: string, siteUrl: string, problems: any[]): Promise<FixResult> {
   ...
   */
  private static async fixServerErrors(userToken: string, siteUrl: string, problems: any[]): Promise<FixResult> {
   ...
   */
  private static generateUpdatedRobotsTxt(siteUrl: string, allowedPaths: string[]): string {
...
Crawl-delay: 1`;
   ...
   */
  private static generateUserSuccessMessage(totalFixed: number, appliedFixes: string[], userMessages: string[]): string {
