...
export interface UrlVariations {
  gscFormat: string;
...
export class UrlNormalizationService {
   ...
   */
  static generateUrlVariations(siteUrl: string): UrlVariations {
    // Clean the base URL
   ...
   */
  static isSameSite(url1: string, url2: string): boolean {
    ...
    return allVariations1.some(v1 => 
      allVariations2.some(v2 => 
        this.normalizeForComparison(v1) === this.normalizeForComparison(v2)
      )
    );
  }
   ...
   */
  static findMatchingSitemap(
    gscSitemapUrl: string, 
    localSitemaps: Array<{site_url: string; sitemap_url: string; [key: string]: any}>
  ): any | null {
    ...
    let match = localSitemaps.find(sitemap => 
      this.normalizeForComparison(sitemap.sitemap_url) === this.normalizeForComparison(gscSitemapUrl)
    );
    ...
    for (const sitemap of localSitemaps) {
      ...
      const hasMatch = allGscVariations.some(gscVar => 
        allLocalVariations.some(localVar => 
          this.normalizeForComparison(gscVar) === this.normalizeForComparison(localVar)
        )
      );
   ...
   */
  private static cleanSiteUrl(siteUrl: string): string {
    return siteUrl
   ...
   */
  private static normalizeForComparison(url: string): string {
    return url
      .toLowerCase()
      .replace(/^https?:\/\//, '')
      .replace(/^www\./, '')
   ...
   */
  static domainPropertyToHttps(domainProperty: string): string {
   ...
   */
  static urlToDomainProperty(url: string): string {
