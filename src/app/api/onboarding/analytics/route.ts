...
export async function GET(request: NextRequest) {
  try {
    ...
    if (buildingError) {
      ...
      return NextResponse.json(
        { error: 'Failed to fetch building methods' },
        { status: 500 }
      )
    }
    ...
    if (cmsError) {
      ...
      return NextResponse.json(
        { error: 'Failed to fetch CMS usage' },
        { status: 500 }
      )
    }
    ...
    if (hostingError) {
      ...
      return NextResponse.json(
        { error: 'Failed to fetch hosting providers' },
        { status: 500 }
      )
    }
    ...
    const cmsTypeCounts = cmsUsage.reduce((acc: any, survey: any) => {
      if (!survey.uses_cms) {
    ...
    const hostingCounts = hostingProviders.reduce((acc: any, survey: any) => {
    ...
    const businessTypeCounts = businessTypes.reduce((acc: any, survey: any) => {
      acc[survey.business_type] = (acc[survey.business_type] || 0) + 1
      return acc
    }, {})

    const websiteAgeCounts = businessTypes.reduce((acc: any, survey: any) => {
      acc[survey.website_age] = (acc[survey.website_age] || 0) + 1
      return acc
    }, {})

    const visitorCounts = businessTypes.reduce((acc: any, survey: any) => {
      acc[survey.monthly_visitors] = (acc[survey.monthly_visitors] || 0) + 1
      return acc
    }, {})

    const seoExperienceCounts = businessTypes.reduce((acc: any, survey: any) => {
      acc[survey.seo_experience] = (acc[survey.seo_experience] || 0) + 1
      return acc
    }, {})

    // Calculate daily completion trend
    const dailyCompletions = completionStats.reduce((acc: any, survey: any) => {
    ...
    const analytics = {
      ...
      trends: {
        dailyCompletions: Object.entries(dailyCompletions).map(([date, stats]: [string, any]) => ({
          date,
          total: stats.total,
      ...
      insights: {
        mostPopularBuildingMethod: Object.entries(buildingMethodCounts).sort(([,a]: any, [,b]: any) => b - a)[0]?.[0] || 'N/A',
        mostPopularCMS: Object.entries(cmsTypeCounts).sort(([,a]: any, [,b]: any) => b - a)[0]?.[0] || 'N/A',
        mostPopularHosting: Object.entries(hostingCounts).sort(([,a]: any, [,b]: any) => b - a)[0]?.[0] || 'N/A',
        mostCommonBusinessType: Object.entries(businessTypeCounts).sort(([,a]: any, [,b]: any) => b - a)[0]?.[0] || 'N/A'
      },
