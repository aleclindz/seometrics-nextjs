...
export async function POST(request: NextRequest) {
  try {
    const body: FreeAuditRequest = await request.json();
    ...
    const overallScore = Math.max(0, Math.min(100, 
      100 - (criticalIssues * 20) - (warningIssues * 10) - (infoIssues * 5)
    ));

    const auditResult: FreeAuditResults = {
      websiteUrl: cleanUrl,
...
async function auditBasicHTML(websiteUrl: string): Promise<AuditIssue[]> {
...
async function auditRobotsTxt(websiteUrl: string): Promise<AuditIssue[]> {
  const issues: AuditIssue[] = [];
...
async function auditSitemap(websiteUrl: string): Promise<AuditIssue[]> {
  const issues: AuditIssue[] = [];
