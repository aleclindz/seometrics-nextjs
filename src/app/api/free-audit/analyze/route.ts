...
import { getCrawlResult } from '@/services/crawl/firecrawl-client';

export const dynamic = 'force-dynamic';

function analyzePages(pages: Array<{ url: string; html?: string; markdown?: string }>) {
  const limit = Math.min(pages.length, 80);
  let missingTitle = 0, missingDesc = 0, noH1 = 0, noCanonical = 0, noindex = 0;
  const samples: { url: string; issues: string[] }[] = [];
  ...
  for (const p of pages.slice(0, limit)) {
    ...
    if (issues.length && samples.length < 10) samples.push({ url: p.url, issues });
  }
  ...
  if (missingTitle > 0) recommendations.push('Add descriptive <title> tags to pages missing titles');
  if (missingDesc > 0) recommendations.push('Write compelling meta descriptions for pages missing descriptions');
  if (noH1 > 0) recommendations.push('Ensure a single, descriptive H1 exists on key pages');
  if (noCanonical > 0) recommendations.push('Add canonical tags to prevent duplicate content issues');
  if (noindex > 0) recommendations.push('Remove unintended noindex directives on important pages');
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ success: true, totals, recommendations, pagesAnalyzed: pages.length });
  } catch (error) {
