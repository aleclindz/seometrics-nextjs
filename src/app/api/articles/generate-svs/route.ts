...
import { SVSOptimizedContentGenerator, SVSOptimizedContentRequest } from '@/services/svs/svs-optimized-content';
import { SemanticAnalyzer } from '@/services/svs/semantic-analyzer';
...
interface GenerateSVSArticleRequest {
  userToken: string;
  websiteToken?: string;
  title: string;
  keywords: string[];
  ...
  targetSVSScore?: number;
  performSVSAnalysis?: boolean; // Whether to analyze the generated content
  saveToDatabase?: boolean; // Whether to save the article
}

export async function POST(request: NextRequest) {
  try {
    const body: GenerateSVSArticleRequest = await request.json();
    const {
      userToken,
      websiteToken,
      title,
    ...
    const contentGenerator = new SVSOptimizedContentGenerator(process.env.OPENAI_API_KEY!);
    
    const contentRequest: SVSOptimizedContentRequest = {
      title,
      keywords,
      websiteDomain,
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        actualSVSAnalysis: actualSVSAnalysis ? {
          ...
          grade: SemanticAnalyzer.getSVSGrade(actualSVSAnalysis.overall_svs_score),
