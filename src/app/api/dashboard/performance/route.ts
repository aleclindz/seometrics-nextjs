...
export async function GET(request: NextRequest) {
  try {
    ...
    const topQueries = (latestData.queries as any[] || []).slice(0, 10).map((q: any) => ({
      query: q.query,
    ...
    // Extract top pages
    const topPages = (latestData.pages as any[] || []).slice(0, 10).map((p: any) => ({
      page: p.page,
    ...
    const referrers = topQueries.slice(0, 5).map((q: any, index: number) => {
