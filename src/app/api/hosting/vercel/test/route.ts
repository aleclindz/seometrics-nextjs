...
import { HostDetectionService } from '@/lib/HostDetectionService';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    try {
      const detectionResult = await HostDetectionService.detectProvider('seoagent.com', 'SEOAgent-Test/1.0');
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  <url>
    ...
    testResults.tests.apiEndpoints = {
      success: Object.values(endpointResults).every((result: any) => result.accessible),
    ...
    const successfulTests = allTests.filter((test: any) => test.success === true).length;
    const failedTests = allTests.filter((test: any) => test.success === false).length;
    const skippedTests = allTests.filter((test: any) => test.success === null).length;
