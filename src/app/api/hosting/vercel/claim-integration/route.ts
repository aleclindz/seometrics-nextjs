  ...
  try {
    ...
    if (!tempIntegrationId || !userToken) {
      return NextResponse.json(
        { error: 'Missing required parameters: tempIntegrationId, userToken' },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { error: 'Internal server error claiming integration' },
      { status: 500 }
    );
  }
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!tempIntegrationId) {
      return NextResponse.json(
        { error: 'Missing tempIntegrationId parameter' },
        { status: 400 }
      );
    }
