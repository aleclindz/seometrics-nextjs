...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: 'Emergency SEO fix failed' }, { status: 500 });
  }
}

async function emergencyIndexabilityFix(siteUrl: string, userToken: string) {
  const fixes: any[] = [];
  const critical: any[] = [];
  const errors: any[] = [];
...
async function emergencyTechnicalFix(siteUrl: string) {
  const fixes: any[] = [];
  const critical: any[] = [];
  const errors: any[] = [];
...
async function emergencyContentFix(siteUrl: string) {
  const fixes: any[] = [];
  const critical: any[] = [];
  const errors: any[] = [];
...
async function deployEmergencyMonitoring(siteUrl: string) {
  const fixes: any[] = [];
...
async function createEmergencyActionItems(userToken: string, siteUrl: string, emergencyResults: any) {
  try {
...
async function logEmergencyFixesToEvents(userToken: string, emergencyResults: any) {
  try {
    // Log all fixes and critical issues
    const allEvents = [
      ...emergencyResults.fixes.map((fix: any) => ({
        user_token: userToken,
      ...
      ...emergencyResults.criticalIssues.map((critical: any) => ({
        user_token: userToken,
        site_url: emergencyResults.siteUrl,
        page_url: emergencyResults.pageUrl,
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('[EMERGENCY-SEO] Error:', error);
    return NextResponse.json({ error: 'Failed to get emergency capabilities' }, { status: 500 });
  }
}