...
export async function POST(request: NextRequest) {
  try {
    ...
    scanResults.issues.forEach(issue => {
      ...
      if (issue.severity === 'critical') scanResults.summary.critical++;
      else if (issue.severity === 'warning') scanResults.summary.warnings++;
      else scanResults.summary.passed++;
    });
...
async function checkIndexability(siteUrl: string, userToken: string) {
  const issues: any[] = [];
...
async function checkCanonicalTags(siteUrl: string) {
  const issues: any[] = [];
...
async function checkMetaTags(siteUrl: string) {
  const issues: any[] = [];
...
async function checkSchemaMarkup(siteUrl: string) {
  const issues: any[] = [];
...
async function checkRobotsTxt(siteUrl: string) {
  const issues: any[] = [];
...
async function storeScanResults(userToken: string, scanResults: any) {
  try {
    // Store in seo_monitoring_events table
    const events = scanResults.issues.map((issue: any) => ({
