...
function shouldCreateActionRecord(functionName: string): boolean {
...
function shouldCreateIdeaRecord(functionName: string): boolean {
  const ideaFunctions = [
    'create_idea',
    'adopt_idea'
  ];
...
async function createIdeaRecord(
  userToken: string,
  functionName: string,
  functionArgs: any, 
  result: any,
...
function generateActionTitle(functionName: string, args: any): string {
  const titleMap: Record<string, (args: any) => string> = {
...
function mapFunctionToActionType(functionName: string): string {
...
function calculateICEScore(args: any): number {
  // Simple scoring based on available data
  let score = 50; // Base score

  if (args.evidence?.length > 0) score += 20; // Has evidence
  if (args.hypothesis?.length > 100) score += 15; // Detailed hypothesis  
  if (args.site_url) score += 10; // Site-specific
  if (args.urgency === 'high') score += 15; // High urgency
