...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ error: 'Failed to fetch memory updates' }, { status: 500 });
    }

    // Filter entries that have conversation source metadata
    const conversationMemories = (memoryEntries || []).filter(entry => 
      entry.metadata?.source && entry.metadata?.message_id
    );

    // Enrich with message details
    const enrichedMemories = await Promise.all(
      conversationMemories.map(async (memory) => {
    ...
    return NextResponse.json(
      { error: 'Failed to fetch memory updates' }, 
      { status: 500 }
    );
  }
}