...
export async function POST(request: NextRequest) {
  try {
      ...
      try {
        ...
        for (const website of websites || []) {
          try {
            ...
            const hasProperty = connection.gsc_properties?.some((prop: any) => 
    ...
    return NextResponse.json(
      { error: 'Failed to run URL inspections sync' }, 
      { status: 500 }
    );
  }
}
...
export async function GET() {
  return NextResponse.json({ message: 'URL Inspections CRON endpoint. Use POST method.' });
}