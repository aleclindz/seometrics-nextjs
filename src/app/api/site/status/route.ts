...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!userToken || !siteUrl) {
      return NextResponse.json({ success: false, error: 'User token and site_url are required' }, { status: 400 });
    }
    ...
    // SmartJS detection
    const smartjsStatus = getSmartJSStatus(cleanDomain) === 'active' ? 'active' : 'inactive';
    ...
    const totals = (dailyRows || []).reduce((acc, r: any) => {
