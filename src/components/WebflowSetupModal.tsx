...
interface WebflowSetupModalProps {
  isOpen: boolean;
  onClose: () => void;
  userToken: string;
  websiteId: number;
...
type Step = 'sites' | 'collections' | 'mapping' | 'publish';

export default function WebflowSetupModal({
  isOpen,
  onClose,
  userToken,
  websiteId,
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-6 py-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
              Step {step === 'sites' ? '1' : step === 'collections' ? '2' : step === 'mapping' ? '3' : '4'} of 4
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="sticky bottom-0 bg-gray-50 dark:bg-gray-700/30 px-6 py-4 flex items-center justify-between border-t border-gray-200 dark:border-gray-700">
          <button
            onClick={handleBack}
            disabled={step === 'sites' || loading}
          ...
          <button
            onClick={step === 'publish' ? handleFinalizeSetup : handleNext}
            disabled={loading}
          ...
          >
            {loading ? 'Loading...' : step === 'publish' ? 'Finish Setup' : 'Continue'}
          </button>
        </div>
      </div>
    </div>
