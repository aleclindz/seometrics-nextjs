...
exports.ContentAbility = void 0;
const base_ability_1 = require("./base-ability");
const content_intelligence_service_1 = require("../../content/content-intelligence-service");
const content_producer_1 = require("@/services/queue/content-producer");
class ContentAbility extends base_ability_1.BaseAbility {
    getFunctionNames() {
        return [
            'generate_article',
     ...
     */
    mapWordCountToLength(wordCount) {
        if (!wordCount)
            return 'medium';
        if (wordCount < 700)
            return 'short';
     ...
     */
    async generateEnhancedArticle(args) {
        try {
            ...
            if (!websiteId && args.site_url) {
                ...
                if (sites?.success && sites.websites?.length) {
                    const clean = (s) => s?.replace(/^https?:\/\//, '').replace(/\/$/, '');
                    const target = clean(args.site_url);
                    const match = sites.websites.find((w) => clean(w.domain) === target);
            ...
            try {
                await (0, content_producer_1.enqueueContentGeneration)(`generate-article-${articleId}`, {
                    userToken: this.userToken,
                    articleId,
                    websiteId,
                    topic: args.topic,
    ...
    async generateAndPublish(args) {
        try {
            ...
            const clean = (s) => s?.replace(/^https?:\/\//, '').replace(/\/$/, '');
            const target = clean(args.site_url);
            const site = sitesResponse.websites.find((s) => clean(s.domain) === target) || sitesResponse.websites[0];
            ...
            if (!createResp?.success)
                return this.error(createResp?.error || 'Failed to create article');
            ...
            if (!genResp?.success)
                return this.error(genResp?.error || 'Article generation failed');
            // Step 3: publish
            let publishDetails = {};
            if (args.publish !== false && cms?.id) {
                ...
                if (!pubResp?.success)
                    return this.error(pubResp?.error || 'Publishing failed');
            ...
            try {
                ...
                const found = (list?.articles || []).find((a) => a.id === articleId);
                if (found?.public_url)
                    links.push({ label: 'View Live', url: found.public_url });
                if (found?.cms_admin_url)
                    links.push({ label: 'Open in CMS', url: found.cms_admin_url });
            }
            catch { }
            const actionCard = {
                type: 'progress',
    ...
    async analyzeContentPerformance(args) {
        try {
            ...
            const filteredArticles = args.article_id
                ? articles.filter((article) => article.id === args.article_id)
                : articles;
            // Calculate basic performance metrics
            const performanceData = filteredArticles.map((article) => ({
                id: article.id,
                title: article.title,
            ...
            return this.success({
                ...
                summary: {
                    ...
                    average_quality_score: performanceData.reduce((sum, a) => sum + (a.quality_score || 0), 0) / performanceData.length,
                    average_seo_score: performanceData.reduce((sum, a) => sum + (a.seo_score || 0), 0) / performanceData.length,
                    total_word_count: performanceData.reduce((sum, a) => sum + (a.word_count || 0), 0)
                }
            });
        }
        catch (error) {
     ...
     */
    async suggestContentIdeas(args) {
        try {
            ...
            const intelligenceService = new content_intelligence_service_1.ContentIntelligenceService();
        ...
        try {
            ...
            if (!topic) {
                ...
                keywords = context.keywords
                    .filter((k) => topic.toLowerCase().includes(k.keyword.toLowerCase()) || k.keyword.toLowerCase().includes(topic.toLowerCase()))
                    .slice(0, 5)
                    .map((k) => k.keyword);
                if (keywords.length === 0) {
                    keywords = context.keywords.slice(0, 3).map((k) => k.keyword);
                }
            }
     ...
     */
    async generateArticleWithInternalLinks(args) {
        try {
            if (!args.topic) {
            ...
            if (keywordStrategyResponse.success && keywordStrategyResponse.hasStrategy) {
                ...
                if (!topicCluster) {
                    ...
                    const matchingCluster = clusters.find((cluster) => cluster.keywords.some((k) => args.topic.toLowerCase().includes(k.keyword.toLowerCase()) ||
                        k.keyword.toLowerCase().includes(args.topic.toLowerCase())));
                    if (matchingCluster) {
                        topicCluster = matchingCluster.name;
                        relatedKeywords = matchingCluster.keywords.slice(0, 5).map((k) => k.keyword);
                    }
                }
                ...
                for (const cluster of clusters) {
                    if (cluster.content && cluster.content.length > 0) {
                        linkableContent = linkableContent.concat(cluster.content.map((content) => ({
                            ...content,
            ...
            if (linkableContent.length > 0 && articleContent) {
                ...
                for (const linkableItem of prioritizedContent) {
                    if (!linkableItem.article_title || !linkableItem.article_url)
                        continue;
                    // Find the best keyword to use as anchor text
                    const anchorKeyword = this.findBestAnchorText(linkableItem.keywords || [], linkableItem.article_title, articleContent);
                    if (anchorKeyword) {
                        // Insert the internal link naturally into the content
                        const linkInserted = this.insertInternalLink(articleContent, linkableItem.article_url, anchorKeyword, linkableItem.article_title);
            ...
            const enhancedResult = {
                ...
                internal_linking: {
                    ...
                    links_by_cluster: internalLinksAdded.reduce((acc, link) => {
    ...
    findBestAnchorText(keywords, articleTitle, content) {
    ...
    insertInternalLink(content, targetUrl, anchorText, targetTitle) {
        try {
    ...
    generateLinkingRecommendations(linksAdded, totalLinkableContent) {
...
exports.ContentAbility = ContentAbility;
