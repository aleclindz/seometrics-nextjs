...
class ConversationAnalyzer {
    ...
    userToken;
    agentMemory;
    constructor(websiteToken, userToken, agentMemory) {
    ...
    async analyzeUserMessage(message) {
        ...
        const preferencePatterns = [
            ...
            // Business type context
            {
                ...
                type: 'context',
                key: 'business_type',
                extractor: (match) => match[2] || match[1]?.trim()
            },
            // SEO focus areas
            {
     ...
     */
    async analyzeAssistantMessage(message, allMessages) {
        const insights = [];
    ...
    async applyInsights(insights) {
        ...
        for (const insight of insights) {
            try {
                const success = await this.agentMemory.storeMemory(insight.type, insight.key, insight.value, {
     ...
     */
    async analyzeAndUpdateMemory(messages) {
        try {
     ...
     */
    async getAnalysisSummary(threadId) {
