...
exports.ContentIntelligenceService = void 0;
...
class ContentIntelligenceService {
     ...
     */
    async getWebsiteContentContext(userToken, siteUrl) {
        // Clean the site URL
     ...
     */
    async getKeywordOpportunities(userToken, domain) {
        try {
            ...
            queryMap.forEach((data) => {
                ...
                if (data.impressions < 10 || data.keyword.length < 3)
                    return;
     ...
     */
    async getCMSInfo(userToken, websiteId) {
        try {
     ...
     */
    async getExistingContent(userToken, websiteId) {
        try {
     ...
     */
    generateTopicSuggestions(keywords, existingContent) {
        const suggestions = [];
        ...
        themes.forEach(theme => {
            ...
            const hasExistingContent = existingContent.some(title => themeWords.some(word => title.includes(word)));
     ...
     */
    groupKeywordsByTheme(keywords) {
        const themes = [];
        const used = new Set();
        keywords.forEach(keyword => {
            if (used.has(keyword.keyword))
                return;
            // Find related keywords
            const related = keywords.filter(k => !used.has(k.keyword) &&
     ...
     */
    generateTitle(keywords, articleType) {
     ...
     */
    identifyContentGaps(keywords, existingContent) {
        const gaps = [];
        // High-impressions keywords without content
        const highOpportunityKeywords = keywords.filter(k => k.potential === 'high' &&
    ...
    async getWebsiteRecord(userToken, domain) {
...
exports.ContentIntelligenceService = ContentIntelligenceService;
