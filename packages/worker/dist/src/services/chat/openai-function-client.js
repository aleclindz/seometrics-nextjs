...
exports.OpenAIFunctionClient = void 0;
...
class OpenAIFunctionClient {
    ...
    agentMemory;
    constructor(apiKey) {
        this.openai = new openai_1.default({ apiKey, dangerouslyAllowBrowser: true });
            ...
            if (messageContent.tool_calls && messageContent.tool_calls.length > 0) {
                ...
                if (toolCall.type === 'function') {
                    ...
                    if (context.userToken) {
                        try {
                            await this.recordFunctionCall(context.userToken, functionName, functionArgs, result, executionTime, context.siteContext?.selectedSite);
                        }
                        catch (dbError) {
                    ...
                    if (this.agentMemory) {
                        try {
                            ...
                            await this.agentMemory.recordAction(functionName, functionArgs, outcome, {
...
- Use a friendly but professional tone`;
    ...
    async recordFunctionCall(userToken, functionName, functionArgs, result, executionTimeMs, siteUrl) {
        try {
...
exports.OpenAIFunctionClient = OpenAIFunctionClient;
class FunctionCaller {
    ...
    async checkSEOAgentJSStatus(args) {
        try {
    ...
    async optimizePageContent(args) {
    ...
    async auditSite(args) {
        try {
            ...
            const site = sites.find((s) => s.url === args.site_url || s.url === args.site_url.replace(/^https?:\/\//, ''));
            if (!site) {
    ...
    analyzeTrends(performanceHistory) {
    ...
    async listIntegrations(args) {
        try {
            ...
            return {
                success: true,
                data: {
                    available_capabilities: result.capabilities.available.map((cap) => ({
                        name: cap.capability_name,
                        category: cap.category,
                        description: cap.description,
                        status: cap.integration_status
                    })),
                    unavailable_capabilities: result.capabilities.unavailable.map((cap) => ({
                        name: cap.capability_name,
                        category: cap.category,
                        description: cap.description,
    ...
    async getTechnicalSEOStatus(args) {
        try {
            const userToken = localStorage.getItem('userToken') || sessionStorage.getItem('userToken');
            if (!userToken) {
                return { success: false, error: 'Authentication required' };
            ...
            return {
                ...
                data: {
                    ...
                    timestamp: new Date().toISOString(),
                    summary: {
                        total_issues: result.data.reduce((acc, website) => acc + (website.issues ? website.issues.length : 0), 0),
                        auto_fixable_issues: result.data.reduce((acc, website) => acc + (website.issues ? website.issues.filter((issue) => issue.auto_fixable).length : 0), 0),
                        critical_issues: result.data.reduce((acc, website) => acc + (website.issues ? website.issues.filter((issue) => issue.severity === 'critical').length : 0), 0)
                    }
                }
            };
        }
    ...
    async checkFixStatus(args) {
        try {
            ...
            if (technicalStatus.success && technicalStatus.data?.technical_status) {
                technicalStatus.data.technical_status.forEach((website) => {
                    if (website.issues) {
    ...
    generateTagRecommendations(stats, smartjsData) {
        const recommendations = [];
    ...
    getFixStatusNextActions(fixes) {
        ...
        if (fixes.manual_fixes_needed.length > 0) {
            const highPriorityManual = fixes.manual_fixes_needed.filter((fix) => fix.priority === 'critical' || fix.priority === 'high');
    ...
    async createSampleActivity(userToken, siteUrl) {
        try {
            await fetch('/api/agent/record-activity', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    async analyzeSemanticVisibility(args) {
        try {
            const userToken = localStorage.getItem('userToken') || sessionStorage.getItem('userToken');
            if (!userToken) {
                return { success: false, error: 'Authentication required' };
                ...
                data: {
                    ...
                    summary: {
                        ...
                        estimated_improvement_potential: result.data.analysis_data.recommendations
                            .reduce((sum, rec) => sum + rec.potential_points, 0)
                    }
                }
            };
        }
    ...
    async getSVSResults(args) {
        try {
            ...
            if (args.limit)
                params.append('limit', args.limit.toString());
            if (args.analysis_id)
                params.append('analysisId', args.analysis_id);
        ...
        try {
            ...
            return {
                ...
                data: {
                    ...
                    message: `Generated ${result.data.wordCount}-word article with SVS score of ${result.data.actualSVSAnalysis?.overall_score || result.data.estimatedSVSScore}/100`
                }
            };
        }
        catch (error) {
    ...
    async analyzeSVSOpportunities(args) {
        try {
            ...
            const analyzer = new (await import('@/services/svs/svs-optimized-content')).SVSOptimizedContentGenerator(process.env.NEXT_PUBLIC_OPENAI_API_KEY || '');
            const opportunities = await analyzer.analyzeSVSOpportunities(args.content, args.title, args.keywords);
            return {
                success: true,
                data: {
                    ...
                    quick_wins: opportunities.improvements
                        ...
                        .slice(0, 5),
                    message: `Found ${opportunities.improvements.length} optimization opportunities with potential ${opportunities.improvements.reduce((sum, imp) => sum + imp.potentialGain, 0)} point SVS improvement`
                }
            };
        }
        catch (error) {
