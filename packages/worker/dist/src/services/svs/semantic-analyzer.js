...
class SemanticAnalyzer {
     ...
     */
    async analyzeSVS(input) {
        const startTime = Date.now();
        try {
            // Run all analysis components in parallel for speed
            const [entityCoverage, semanticVariety, qaUtility, citationEvidence, claritySimplicity, topicDepth, structureSchema] = await Promise.all([
            ...
            const overallScore = entityCoverage.score +
     ...
     */
    async analyzeEntityCoverage(input) {
...
Respond with JSON:
{
  ...
  "recommendations": [
     ...
     */
    async analyzeSemanticVariety(input) {
...
Respond with JSON:
{
  ...
  "recommendations": [...]
     ...
     */
    async analyzeCitationEvidence(input) {
...
Respond with JSON:
{
  ...
  "recommendations": [...]
     ...
     */
    analyzeClaritySimplicity(input) {
        return new Promise((resolve) => {
            try {
                ...
                if (avgSentenceLength > 25)
                    score -= 2; // Very long sentences
                else if (avgSentenceLength > 20)
                    score -= 1; // Long sentences
                if (complexWordsRatio > 0.3)
                    score -= 2; // High complexity
                else if (complexWordsRatio > 0.2)
...
Respond with JSON:
{
  ...
  "recommendations": [...]
    ...
    analyzeStructureSchema(input) {
        return new Promise((resolve) => {
            try {
                ...
                const foundSemanticElements = semanticElements.filter(element => new RegExp(`<${element}`, 'gi').test(html));
     ...
     */
    static getSVSGrade(score) {
        if (score >= 90)
            return { grade: 'A+', label: 'Exceptional', color: '#10B981' };
        if (score >= 85)
            return { grade: 'A', label: 'Excellent', color: '#34D399' };
        if (score >= 80)
            return { grade: 'A-', label: 'Very Good', color: '#6EE7B7' };
        if (score >= 75)
            return { grade: 'B+', label: 'Good', color: '#84CC16' };
        if (score >= 70)
            return { grade: 'B', label: 'Above Average', color: '#A3E635' };
        if (score >= 65)
            return { grade: 'B-', label: 'Average', color: '#FBBF24' };
        if (score >= 60)
            return { grade: 'C+', label: 'Below Average', color: '#F59E0B' };
        if (score >= 55)
            return { grade: 'C', label: 'Needs Work', color: '#F97316' };
        if (score >= 50)
            return { grade: 'C-', label: 'Poor', color: '#EF4444' };
        if (score >= 40)
            return { grade: 'D', label: 'Very Poor', color: '#DC2626' };
