...
exports.HostDetectionService = void 0;
class HostDetectionService {
    // Import database integration
    static dbEnabled = null;
    static PROVIDER_FINGERPRINTS = {
        cloudflare: {
            headers: {
     ...
     */
    static async checkDatabaseAvailability() {
     ...
     */
    static async detectProvider(domain, userAgent = 'SEOAgent-HostDetection/1.0', userToken) {
        ...
        const providers = Array.from(detectedProviders.entries())
        ...
        if (dbEnabled && userToken) {
            try {
                ...
                await HostingProviderDatabase.saveDetectionResult(userToken, url, domain, providers.map(p => ({ name: p.name, type: p.type, confidence: p.confidence })), primaryProvider?.name || null, overallConfidence, detectionMethods, matchedFingerprints, detectionDuration, userAgent);
            }
            catch (error) {
     ...
     */
    static async detectViaHeaders(url, userAgent, dbEnabled = false) {
     ...
     */
    static async detectViaDNS(domain, dbEnabled = false) {
        const results = new Map();
        const matchedFingerprints = [];
     ...
     */
    static async detectViaPathFingerprints(url, userAgent, dbEnabled = false) {
        const results = new Map();
        const matchedFingerprints = [];
        try {
                ...
                try {
                    ...
                    for (const [provider, fingerprints] of Object.entries(providerPaths)) {
                        ...
                        for (const fp of fingerprints) {
                            ...
                            const expectedStatuses = fp.fingerprint_value.split(',').map((s) => parseInt(s.trim()));
                            try {
            ...
            if (!dbEnabled || results.size === 0) {
                for (const [provider, fingerprint] of Object.entries(this.PROVIDER_FINGERPRINTS)) {
                    if (!fingerprint.pathFingerprints)
                        continue;
                    let matches = 0;
     ...
     */
    static generateRecommendations(providers, domain) {
     ...
     */
    static matchesDatabasePattern(value, fingerprint) {
     ...
     */
    static getIntegrationInstructions(provider, domain) {
...
exports.HostDetectionService = HostDetectionService;
