...
exports.CodingAgentInstructionsGenerator = void 0;
class CodingAgentInstructionsGenerator {
    static generate(issueType, issueTitle, affectedUrls, metadata) {
...
export default function Contact() {
  return (
     ...
     */
    static generateSpecificIndexingInstructions(issueTitle, affectedUrls, metadata) {
        ...
        const problemDescriptions = Object.entries(problemsByType).map(([problemType, problems]) => {
        ...
        if (problemsByType['Page Not Found']) {
            ...
            const pageAnalysis = notFoundPages.map((p) => urlAnalysis.find(a => a.url === p.url)).filter(Boolean);
            specificInstructions.push('Fix 404 "Page Not Found" errors by creating the missing pages', 'Each page should have proper content, meta tags, and navigation');
...
export const metadata = {
  ...
  description: 'Description for the ${page.pageName} page',
        ...
        if (problemsByType['Robots.txt Blocking']) {
            specificInstructions.push('Update robots.txt to allow access to blocked pages', 'Ensure search engines can crawl important content pages');
            codeExamples.push(`
...
${problemsByType['Robots.txt Blocking'].map((p) => {
                try {
...
Sitemap: https://yoursite.com/sitemap.xml`);
        ...
        if (problemsByType['Access Denied']) {
            specificInstructions.push('Configure server to allow search engine access', 'Remove password protection or IP restrictions for public pages');
        }
    ...
    static generateCopyableInstructions(instructions) {
...
exports.CodingAgentInstructionsGenerator = CodingAgentInstructionsGenerator;
