...
exports.ShopifyProvider = void 0;
const base_provider_1 = require("../base-provider");
class ShopifyProvider extends base_provider_1.BaseCMSProvider {
    type = 'shopify';
    name = 'Shopify';
    apiVersion = '2024-10';
    ...
    getAuthUrl(redirectUri, state, shopDomain) {
    ...
    async getBlogs(credentials) {
        const blogsResponse = await this.makeAuthenticatedRequest(`/admin/api/${this.apiVersion}/blogs.json`, 'GET', credentials);
    ...
    async getArticles(credentials, blogId) {
        ...
        const articlesResponse = await this.makeAuthenticatedRequest(`/admin/api/${this.apiVersion}/blogs/${blogId}/articles.json?limit=50`, 'GET', credentials);
    ...
    async publishArticle(credentials, article, options) {
        ...
        const createdArticle = await this.makeAuthenticatedRequest(`/admin/api/${this.apiVersion}/blogs/${options.blogId}/articles.json`, 'POST', credentials, articleData);
    ...
    async updateArticle(credentials, articleId, article) {
        ...
        if (article.title)
            updateData.article.title = article.title;
        if (article.content)
            updateData.article.body_html = article.content;
        if (article.slug)
            updateData.article.handle = article.slug;
        if (article.excerpt)
            updateData.article.summary = article.excerpt;
        if (article.status) {
            updateData.article.published_at = article.status === 'published'
                ? new Date().toISOString()
                : null;
        }
        const updatedArticle = await this.makeAuthenticatedRequest(`/admin/api/${this.apiVersion}/blogs/${currentArticle.blogId}/articles/${articleId}.json`, 'PUT', credentials, updateData);
    ...
    async deleteArticle(credentials, articleId) {
        // First, get the article to find its blog ID
        const currentArticle = await this.getArticleWithBlogId(credentials, articleId);
        await this.makeAuthenticatedRequest(`/admin/api/${this.apiVersion}/blogs/${currentArticle.blogId}/articles/${articleId}.json`, 'DELETE', credentials);
    }
    ...
    async getArticleWithBlogId(credentials, articleId) {
        // Since we need to know which blog an article belongs to for updates/deletes,
        // we'll search through all blogs to find it
        const blogs = await this.getBlogs(credentials);
        for (const blog of blogs) {
            try {
                await this.makeAuthenticatedRequest(`/admin/api/${this.apiVersion}/blogs/${blog.id}/articles/${articleId}.json`, 'GET', credentials);
    ...
    async verifyWebhook(body, hmacHeader) {
