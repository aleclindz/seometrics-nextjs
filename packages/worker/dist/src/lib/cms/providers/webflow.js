...
exports.WebflowProvider = void 0;
const base_provider_1 = require("../base-provider");
class WebflowProvider extends base_provider_1.BaseCMSProvider {
    type = 'webflow';
    name = 'Webflow';
    constructor(clientId, clientSecret) {
        super(clientId, clientSecret, 'https://api.webflow.com');
    }
    getAuthUrl(redirectUri, state, ...args) {
    ...
    async getBlogs(credentials) {
        ...
        for (const site of sites) {
            ...
            const collectionsResponse = await this.makeAuthenticatedRequest(`/v2/sites/${site.id}/collections`, 'GET', credentials);
            const collections = collectionsResponse.collections || [];
            // Find blog-like collections (usually contain "blog", "post", "article" in name)
            const blogCollections = collections.filter((collection) => /blog|post|article|news/i.test(collection.displayName || collection.singularName));
    ...
    async getArticles(credentials, blogId) {
        ...
        const itemsResponse = await this.makeAuthenticatedRequest(`/v2/collections/${collectionId}/items`, 'GET', credentials);
    ...
    async publishArticle(credentials, article, options) {
        ...
        const collectionResponse = await this.makeAuthenticatedRequest(`/v2/collections/${collectionId}`, 'GET', credentials);
        ...
        const createdItem = await this.makeAuthenticatedRequest(`/v2/collections/${collectionId}/items`, 'POST', credentials, itemData);
        // If not a draft, publish the site to make changes live
        if (options?.status !== 'draft') {
            try {
                await this.makeAuthenticatedRequest(`/v2/sites/${siteId}/publish`, 'POST', credentials);
            }
            catch (error) {
    ...
    async updateArticle(credentials, articleId, article) {
        ...
        const currentItem = await this.makeAuthenticatedRequest(`/v2/collections/items/${articleId}`, 'GET', credentials);
        // Get collection schema
        const collectionResponse = await this.makeAuthenticatedRequest(`/v2/collections/${currentItem.collectionId}`, 'GET', credentials);
        ...
        const updatedItem = await this.makeAuthenticatedRequest(`/v2/collections/items/${articleId}`, 'PATCH', credentials, updateData);
        return {
            id: updatedItem.id,
    ...
    async deleteArticle(credentials, articleId) {
        await this.makeAuthenticatedRequest(`/v2/collections/items/${articleId}`, 'DELETE', credentials);
    }
    // Webflow-specific helper methods
    async makeAuthenticatedRequest(endpoint, method, credentials, body) {
    ...
    mapArticleToWebflowFields(article, collection, options) {
        const fieldData = {};
...
exports.WebflowProvider = WebflowProvider;
