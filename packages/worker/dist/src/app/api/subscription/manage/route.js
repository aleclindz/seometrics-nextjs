    ...
    try {
        ...
        if (error && error.code === 'PGRST116') { // No rows returned
            ...
            if (insertError) {
                ...
                return server_1.NextResponse.json({ error: 'Failed to create user plan' }, { status: 500 });
            }
            userPlan = newPlan;
        }
        else if (error) {
            console.error('Error fetching user plan:', error);
            return server_1.NextResponse.json({ error: 'User plan not found' }, { status: 404 });
        }
...
async function POST(request) {
    try {
        ...
        if (!action || !userToken) {
            return server_1.NextResponse.json({ error: 'Missing required fields: action, userToken' }, { status: 400 });
        }
        // First check login_users.plan to determine if user should be on free tier
        const { data: loginUser, error: loginUserError } = await supabase
            .from('login_users')
...
async function handleCancelSubscription(userPlan) {
    if (!userPlan.stripe_subscription_id) {
        return server_1.NextResponse.json({ error: 'No active subscription to cancel' }, { status: 400 });
    }
    try {
        // Cancel subscription at period end
        const subscription = await stripe.subscriptions.update(userPlan.stripe_subscription_id, {
        ...
        return server_1.NextResponse.json({ error: 'Failed to cancel subscription' }, { status: 500 });
    }
}
async function handleReactivateSubscription(userPlan) {
    if (!userPlan.stripe_subscription_id) {
        return server_1.NextResponse.json({ error: 'No subscription to reactivate' }, { status: 400 });
    }
    try {
        ...
        return server_1.NextResponse.json({ error: 'Failed to reactivate subscription' }, { status: 500 });
    }
}
async function handleCreatePortalSession(userPlan) {
