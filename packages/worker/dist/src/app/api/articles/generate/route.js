...
async function POST(request) {
    try {
        ...
        const { userToken, articleId, targetKeywords = [], contentLength = 'medium', // short, medium, long
        tone = 'professional', // professional, casual, technical
        includeImages = true, 
        // NEW: Enhanced generation options
        articleType = 'blog', // how_to | listicle | guide | faq | comparison | evergreen | blog
        includeCitations = true, referenceStyle = 'link', // link | apa
        numImages = 2, imageProvider = 'openai', // openai | stability | unsplash
        imageStyle = 'clean, modern, web illustration, professional' } = body;
        ...
        if (!article) {
            ...
            return new Response(JSON.stringify({ error: 'Article not found (retry exhausted)' }), { status: 404, headers: { 'Content-Type': 'application/json' } });
        }
        ...
        try {
            ...
            return new Response(JSON.stringify({ error: 'Article generation failed' }), { status: 500, headers: { 'Content-Type': 'application/json' } });
        }
    }
    catch (error) {
...
function calculateQualityScore(content, keywords, extras) {
    let score = 7.0;
    ...
    keywords.forEach(keyword => {
        ...
        if (keywordCount > 0 && keywordCount < 12)
            score += 0.3;
    });
    // Structure scoring
    ...
    if (extras?.citationsCount && extras.citationsCount >= 2)
        score += 0.6;
    if (extras?.hasSchema)
        score += 0.3;
    if (extras?.hasFaq)
        score += 0.2;
    if (extras?.imagesCount && extras.imagesCount >= 2)
        score += 0.2;
...
function calculateSeoScore(content, keywords, title) {
    let score = 6.0;
    ...
    if ((content.match(/<h2/gi) || []).length >= 3)
        score += 0.3;
    if ((content.match(/<ul|<ol/gi) || []).length >= 2)
        score += 0.3;
    return Math.min(10, Math.max(1, Number(score.toFixed(2))));
}
// === EXISTING HELPER FUNCTIONS ===
async function checkQuota(supabase, userToken, siteId) {
    try {
...
async function trackUsage(supabase, userToken, resourceType, siteId) {
    try {
        const currentMonth = new Date().toISOString().slice(0, 7);
