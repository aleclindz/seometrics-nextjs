...
async function POST(request) {
    try {
        ...
        const { userToken, articleId, publishDraft = true // Whether to publish as draft or immediately publish
         } = body;
        if (!userToken || !articleId) {
            return new Response(JSON.stringify({ error: 'Missing required fields: userToken, articleId' }), { status: 400, headers: { 'Content-Type': 'application/json' } });
        }
      ...
      `)
        ...
        if (!article.article_content) {
            return new Response(JSON.stringify({ error: 'Article content not generated yet' }), { status: 400, headers: { 'Content-Type': 'application/json' } });
        }
        if (!article.cms_connections) {
            return new Response(JSON.stringify({ error: 'No CMS connection found for this article' }), { status: 400, headers: { 'Content-Type': 'application/json' } });
        }
        ...
        try {
            ...
            if (article.cms_connection_id && article.cms_connections?.cms_type !== 'strapi') {
                ...
                try {
                    ...
                    const publishedArticle = await provider.publishArticle(connection.credentials, articleData, {
            ...
            const strapiAdminUrl = generateStrapiAdminUrl(article.cms_connections.base_url, article.cms_connections.content_type, cmsArticleId);
            // Generate public URL for the published article
            const publicUrl = generatePublicArticleUrl(article.cms_connections.base_url, article.slug || generateOptimizedSlug(article.title));
...
async function publishToStrapi({ baseUrl, apiToken, contentType, title, content, slug, metaTitle, metaDescription, publishDraft, cmsConnectionId }) {
...
function formatContentForStrapi(content) {
...
async function getContentTypeSchema(connectionId, contentType) {
    try {
...
function formatContentForPublication(content, schemaInfo) {
...
function generateStrapiAdminUrl(baseUrl, contentType, documentId) {
...
function generatePublicArticleUrl(baseUrl, slug) {
    // Generate public URL for the published article
    const cleanUrl = baseUrl.replace(/\/$/, '');
