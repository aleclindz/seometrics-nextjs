    ...
    try {
        ...
        if (type === 'wordpress') {
            ...
            if (siteUrl && userLogin && password) {
                ...
                const result = await cmsManager.completeOAuthFlow(type, 'wordpress-credentials', // Not used for WordPress
                state, storedUserId);
                return new Response(`<html><body><script>
            window.parent.postMessage({
        ...
        const result = await cmsManager.completeOAuthFlow(type, code, state, userId);
