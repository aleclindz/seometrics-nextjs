...
async function POST(request) {
    try {
        ...
        let { cms_type, base_url, api_token, content_type, connection_id, userToken } = body;
        ...
        if (!cms_type || !base_url || !actualApiToken) {
            return server_1.NextResponse.json({ error: 'Missing required fields: cms_type, base_url, api_token' }, { status: 400 });
        }
        ...
        if (cms_type === 'strapi') {
            ...
            return server_1.NextResponse.json({ error: `CMS type '${cms_type}' is not yet supported` }, { status: 400 });
        }
...
function categorizeContentType(displayName, attributes, fieldCount) {
    const name = displayName.toLowerCase();
    if (fieldCount === 0)
        return 'empty';
    if (name.includes('blog') || name.includes('post') || name.includes('article'))
        return 'blog';
    if (name.includes('page') || name.includes('content'))
        return 'page';
    if (name.includes('user') || name.includes('author') || name.includes('profile'))
        return 'user';
    if (name.includes('category') || name.includes('tag') || name.includes('taxonomy'))
        return 'taxonomy';
    if (name.includes('media') || name.includes('image') || name.includes('file'))
        return 'media';
    if (name.includes('comment') || name.includes('review'))
        return 'interaction';
    return 'other';
}
function isSuitableForBlogging(attributes, fieldCount) {
    if (fieldCount === 0)
        return 0; // Empty content types get lowest priority
    let score = 0;
    const attributeValues = Object.values(attributes);
    // High priority indicators
    if (attributeValues.some((attr) => attr.type === 'richtext'))
        score += 10;
    if (attributeValues.some((attr) => attr.type === 'text'))
        score += 7;
    if (attributeValues.some((attr) => attr.type === 'string' && attr.required))
        score += 5;
    if (attributeValues.some((attr) => attr.type === 'uid'))
        score += 3;
    if (attributeValues.some((attr) => attr.type === 'media'))
        score += 3;
    // Field count bonus
    if (fieldCount >= 3)
        score += 5;
...
async function testWixConnection(siteId, apiToken, contentType = 'blog-posts') {
    try {
...
async function testStrapiConnection(baseUrl, apiToken, contentType = 'api::article::article') {
            ...
            if (schemaResponse.ok) {
                ...
                if (discoveredSchemas?.data) {
                    discoveredContentTypes = discoveredSchemas.data
                        .filter((contentType) => contentType.uid.startsWith('api::') && // User content types
                    ...
                    // Remove the attribute length filter to catch all user content types
                        ...
                        return {
                            ...
                            hasRichText: Object.values(attributes).some((attr) => attr.type === 'richtext'),
                            hasMedia: Object.values(attributes).some((attr) => attr.type === 'media'),
                            hasUID: Object.values(attributes).some((attr) => attr.type === 'uid'),
                            hasString: Object.values(attributes).some((attr) => attr.type === 'string'),
                            hasText: Object.values(attributes).some((attr) => attr.type === 'text'),
                            hasRelation: Object.values(attributes).some((attr) => attr.type === 'relation'),
...
async function updateConnectionStatus(connectionId, userToken, success, message, schemas) {
    try {
        const updateData = {
...
async function storeContentSchemas(connectionId, schemas) {
    try {
        ...
        const schemaEntries = Object.entries(schemas.data).map(([contentTypeName, schemaData]) => {
...
function processSchemaFields(schemaData) {
    try {
        ...
        return {
            ...
            hasRichText: Object.values(fields).some((field) => field.type === 'richtext'),
            hasMedia: Object.values(fields).some((field) => field.type === 'media'),
            hasRelations: Object.values(fields).some((field) => field.type === 'relation'),
...
async function updateWebsiteCMSStatus(userToken, baseUrl, success) {
    try {
