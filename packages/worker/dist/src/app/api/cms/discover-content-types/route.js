...
async function POST(request) {
    try {
        ...
        if (!base_url || !api_token) {
            return server_1.NextResponse.json({ error: 'Missing required fields: base_url and api_token' }, { status: 400 });
        }
        if (!userToken) {
        ...
        if (!connectionWorking) {
            return server_1.NextResponse.json({ error: connectionError }, { status: 401 });
        }
        ...
        const verifiedTypes = discoveredTypes
            ...
            if (a.id.includes('blog') && !b.id.includes('blog'))
                return -1;
            if (!a.id.includes('blog') && b.id.includes('blog'))
                return 1;
            if (a.id.includes('article') && !b.id.includes('article'))
                return -1;
            if (!a.id.includes('article') && b.id.includes('article'))
                return 1;
            // Show non-empty collections first, but include empty ones
            if ((a.entryCount || 0) > 0 && (b.entryCount || 0) === 0)
                return -1;
            if ((a.entryCount || 0) === 0 && (b.entryCount || 0) > 0)
                return 1;
        ...
        return server_1.NextResponse.json({ error: 'Internal server error during content type discovery' }, { status: 500 });
    }
}
