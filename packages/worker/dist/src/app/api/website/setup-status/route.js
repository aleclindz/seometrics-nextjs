...
async function GET(request) {
    try {
        ...
        const websiteResult = await DomainQueryService_1.DomainQueryService.queryTableByDomain('websites', userToken, domain, 'gsc_status, seoagentjs_status, cms_status, hosting_status, last_status_check, domain, cleaned_domain');
        ...
        const completedCount = statuses.filter(status => status === 'connected' || status === 'active').length;
...
async function POST(request) {
    try {
        const { userToken, domain, gscStatus, seoagentjsStatus, cmsStatus, hostingStatus } = await request.json();
        if (!userToken || !domain) {
            return server_1.NextResponse.json({ error: 'Missing required parameters' }, { status: 400 });
        }
        ...
        if (error) {
            ...
            return server_1.NextResponse.json({ error: 'Failed to update setup status' }, { status: 500 });
        }
...
async function PUT(request) {
    try {
        const { userToken, domain, setupDismissed } = await request.json();
        if (!userToken || !domain) {
            return server_1.NextResponse.json({ error: 'Missing required parameters' }, { status: 400 });
        }
        // Update setup preferences - first try to find existing record with DomainQueryService
        const existingStatusResult = await DomainQueryService_1.DomainQueryService.queryTableByDomain('website_setup_status', userToken, domain, '*');
        ...
        if (error) {
            ...
            return server_1.NextResponse.json({ error: 'Failed to update setup preferences' }, { status: 500 });
        }
        return server_1.NextResponse.json({
            success: true,
            data
