...
async function POST(request) {
    try {
        ...
        if (propertyError) {
            ...
            return server_1.NextResponse.json({ error: 'Database error checking property access' }, { status: 500 });
        }
        if (!property) {
        ...
        try {
            ...
            if (apiError.code === 403) {
                return server_1.NextResponse.json({ error: 'Access denied to GSC property. Please reconnect your Google Search Console.' }, { status: 403 });
            }
            else if (apiError.code === 401) {
                return server_1.NextResponse.json({ error: 'Authentication expired. Please reconnect your Google Search Console.' }, { status: 401 });
            }
            else {
                return server_1.NextResponse.json({ error: `Google API error: ${apiError.message || 'Unknown error'}` }, { status: 500 });
            }
        }
