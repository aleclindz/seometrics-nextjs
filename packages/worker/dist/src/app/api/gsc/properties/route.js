    ...
    try {
        ...
        for (const site of sites) {
            if (site.siteUrl) {
                ...
                const websiteResult = await DomainQueryService_1.DomainQueryService.findWebsiteByDomain(userToken, domain, 'website_token, is_excluded_from_sync, domain, is_managed, meta_tags, image_tags');
        ...
        for (const site of sites) {
            if (site.siteUrl) {
                ...
                const websiteResult = await DomainQueryService_1.DomainQueryService.findWebsiteByDomain(userToken, cleanDomain, 'is_managed');
        ...
        return server_1.NextResponse.json({ error: 'Failed to fetch properties' }, { status: 500 });
    }
}
async function POST(request) {
    try {
        ...
        return server_1.NextResponse.json({ error: 'Failed to refresh properties' }, { status: 500 });
    }
}
