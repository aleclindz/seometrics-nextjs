...
async function POST(request) {
    try {
        ...
        if (!siteUrl || !urls || !Array.isArray(urls) || !userToken) {
            return server_1.NextResponse.json({ error: 'Missing required parameters: siteUrl, urls array, userToken' }, { status: 400 });
        }
        // Limit to 50 URLs per request to avoid timeout
        if (urls.length > 50) {
            return server_1.NextResponse.json({ error: 'Maximum 50 URLs allowed per request' }, { status: 400 });
        }
        ...
        if (propertyError || !property) {
            ...
            return server_1.NextResponse.json({ error: 'Property not found or no access' }, { status: 403 });
        }
        ...
        const inspectUrlWithTimeout = async (url, siteUrl, timeoutMs = 10000) => {
            return Promise.race([
                ...
                new Promise((_, reject) => setTimeout(() => reject(new Error(`URL inspection timeout after ${timeoutMs}ms`)), timeoutMs))
            ]);
        };
        for (const url of urls) {
            try {
                ...
                const inspectionResponse = await inspectUrlWithTimeout(url, siteUrl, 8000);
                ...
                console.error(`[GSC URL INSPECTION] Full error details for ${url}:`, {
                    ...
                    response: urlError?.response?.data || urlError?.response || 'No response data'
                });
        ...
        return server_1.NextResponse.json({ error: 'Failed to perform URL inspection' }, { status: 500 });
    }
}
