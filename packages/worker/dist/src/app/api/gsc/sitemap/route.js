...
async function GET(request) {
    try {
        ...
        return server_1.NextResponse.json({ error: 'Failed to fetch sitemaps' }, { status: 500 });
    }
}
...
async function POST(request) {
    try {
        ...
        if (!siteUrl || !sitemapUrl || !userToken) {
            return server_1.NextResponse.json({ error: 'Missing required parameters: siteUrl, sitemapUrl, userToken' }, { status: 400 });
        }
        ...
        try {
            ...
            return server_1.NextResponse.json({ error: 'Invalid sitemap URL format' }, { status: 400 });
        }
        // Get active GSC connection
        const { data: connection, error: connectionError } = await supabase
            .from('gsc_connections')
        ...
        if (error instanceof Error && error.message.includes('quota')) {
            return server_1.NextResponse.json({ error: 'Google API quota exceeded. Please try again later.' }, { status: 429 });
        }
        return server_1.NextResponse.json({ error: 'Failed to submit sitemap' }, { status: 500 });
    }
}
...
async function DELETE(request) {
    try {
        ...
        return server_1.NextResponse.json({ error: 'Failed to delete sitemap' }, { status: 500 });
    }
}
