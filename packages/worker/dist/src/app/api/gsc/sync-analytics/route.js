...
async function POST(request) {
    try {
        ...
        if (!userToken || !siteUrl) {
            return server_1.NextResponse.json({ error: 'User token and site URL are required' }, { status: 400 });
        }
        ...
        switch (syncType) {
            case 'yesterday':
                result = await (0, gsc_search_analytics_sync_1.syncYesterdayGSCData)(userToken, siteUrl);
                break;
            case 'recent':
                const days = Math.max(1, Math.min(daysBack || 7, 30)); // Limit to 30 days max
                result = await (0, gsc_search_analytics_sync_1.syncRecentGSCData)(userToken, siteUrl, days);
                break;
            default:
                return server_1.NextResponse.json({ error: 'Invalid sync type. Use "yesterday" or "recent"' }, { status: 400 });
        }
        if (result.success) {
