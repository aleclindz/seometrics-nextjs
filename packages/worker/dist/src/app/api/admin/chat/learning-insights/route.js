...
async function GET(request) {
    try {
        ...
        if (learningError) {
            ...
            return server_1.NextResponse.json({ error: 'Failed to fetch learning insights' }, { status: 500 });
        }
        ...
        const memoryGrowth = (memoryData || []).reduce((acc, entry) => {
            ...
            if (!acc[date])
                acc[date] = { context: 0, patterns: 0, preferences: 0, insights: 0, strategies: 0 };
            acc[date][entry.memory_type]++;
            return acc;
        }, {});
        // Get conversation analysis stats
        const conversationMemories = (memoryData || []).filter(entry => entry.metadata?.source === 'explicit' ||
