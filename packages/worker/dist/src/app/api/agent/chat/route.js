...
async function storeConversationMessage(userToken, websiteToken, conversationId, messageRole, messageContent, messageOrder, functionCall, actionCard, metadata) {
    try {
        await supabase
...
async function getNextMessageOrder(userToken, websiteToken, conversationId) {
    try {
        ...
        const lastOrder = data && data.length > 0 ? data[0].message_order : 0;
        return lastOrder + 1;
    }
    catch (err) {
...
function getOrCreateConversationId(conversationHistory) {
...
async function POST(request) {
    try {
        ...
        await storeConversationMessage(userToken, websiteToken, conversationId, 'user', message, userMessageOrder);
        ...
        if (!apiKey) {
            ...
            await storeConversationMessage(userToken, websiteToken, conversationId, 'assistant', testResponse.message, assistantMessageOrder, testResponse.functionCall, testResponse.actionCard, { test_response: true });
            return server_1.NextResponse.json({
                success: true,
        ...
        const functionSchemas = (0, function_schemas_1.getFunctionSchemas)();
        ...
        while (guard++ < MAX_TOOL_STEPS) {
            ...
            if (!messageContent.tool_calls?.length) {
                ...
                return server_1.NextResponse.json(processedResponse);
            }
            ...
            await Promise.all(messageContent.tool_calls.map(async (toolCall) => {
                ...
                try {
                    if (!('conversation_id' in functionArgs) && (clientConversationId || conversationId)) {
                        functionArgs.conversation_id = clientConversationId || conversationId;
                    }
                }
                catch { }
            ...
            const processedResponse = await processOpenAIResponse(immediateResponse, userToken, selectedSite, {
                conversationId,
                websiteToken,
                messageOrder: assistantMessageOrder
            });
...
async function recordActivity(userToken, siteUrl, functionCall, actionCard) {
    try {
        const supabase = (0, supabase_js_1.createClient)(process.env.NEXT_PUBLIC_SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY);
        // Record an agent event
        const eventData = {
            title: functionCall.name.replace(/_/g, ' ').replace(/\b\w/g, (l) => l.toUpperCase()),
...
function getFunctionDisplayName(functionName) {
    switch (functionName) {
        ...
        default:
            return functionName.replace(/_/g, ' ').replace(/\b\w/g, (l) => l.toUpperCase());
    }
}
function getDescriptionForFunction(functionName) {
    switch (functionName) {
        // Core GSC functions
        case 'connect_gsc':
...
async function processOpenAIResponse(response, userToken, selectedSite, conversationData) {
    let actionCard = null;
    ...
    if (response.function_call || (response.tool_calls && response.tool_calls.length > 0)) {
        ...
        if (functionCall.name === 'KEYWORDS_brainstorm' || functionCall.name === 'brainstorm_keywords') {
            ...
            const toolData = functionCall.result;
    ...
    if (conversationData) {
        storeConversationMessage(userToken, conversationData.websiteToken, conversationData.conversationId, 'assistant', response.content, conversationData.messageOrder, functionCall, actionCard, { steps: response.steps }).catch(error => {
...
function buildSystemPrompt(userToken, selectedSite) {
    return (0, prompts_1.getPromptManager)().getPrompt('agent', 'SIMPLE_SEO_AGENT', {
        selectedSite: selectedSite ? `\n\n**Currently Selected Site**: ${selectedSite}` : ''
    });
}
// Build a concise, human-readable summary of executed tool calls
function buildToolSummary(executed, results) {
    ...
    if (first.name === 'KEYWORDS_brainstorm_auto') {
        ...
        const top = ideas.slice(0, Math.min(10, ideas.length));
        const bullets = top.map((k) => `- ${k.keyword || k}`).join('\n');
