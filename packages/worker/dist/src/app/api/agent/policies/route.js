...
const policy_engine_1 = require("@/services/agent/policy-engine");
...
async function GET(request) {
    try {
        ...
        const newSitePolicy = policy_engine_1.PolicyEngine.getNewSitePolicy();
        ...
        if (actionType) {
            ...
            actionPolicy = {
                actionType,
                recommendedPolicy: policy_engine_1.PolicyEngine.getNewSitePolicy(),
        ...
        if (siteUrl && actionType) {
            const hasPermission = await policy_engine_1.PolicyEngine.checkUserPermissions(userToken, siteUrl, actionType);
            permissions = {
                hasPermission,
                siteUrl,
                actionType
...
async function POST(request) {
    try {
        ...
        const { userToken, actionId, actionType, siteUrl, payload, requestedPolicy, targetUrls } = body;
        if (!userToken || !actionType || !siteUrl) {
            return server_1.NextResponse.json({ error: 'Missing required fields: userToken, actionType, siteUrl' }, { status: 400 });
        }
        const context = {
        ...
        // Validate the policy
        const validation = await policy_engine_1.PolicyEngine.validatePolicy(context, requestedPolicy || {});
        let approvalRequestId = null;
        if (validation.approvalRequired && validation.adjustedPolicy) {
            try {
                approvalRequestId = await policy_engine_1.PolicyEngine.createApprovalRequest(context, validation.adjustedPolicy);
            }
            catch (error) {
...
function generatePolicyRecommendations(userToken, siteUrl, actionType) {
...
function generateExecutionRecommendations(policy) {
    const recommendations = [];
...
function generateRejectionRecommendations(reason) {
    const recommendations = [];
