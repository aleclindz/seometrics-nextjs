...
async function GET(request) {
    try {
        ...
        if (error) {
            ...
            return server_1.NextResponse.json({ error: 'Failed to fetch activity' }, { status: 500 });
        }
        ...
        if (actionsResult.error || ideasResult.error) {
            ...
            return server_1.NextResponse.json({ error: 'Failed to fetch current state' }, { status: 500 });
        }
...
function processActivity(events, currentActions, currentIdeas) {
    ...
    const actionStatusCounts = currentActions.reduce((acc, action) => {
        acc[action.status] = (acc[action.status] || 0) + 1;
        return acc;
    }, {});
    const ideaStatusCounts = currentIdeas.reduce((acc, idea) => {
    ...
    return {
        ...
        activity_counts: {
            ...
            completed_actions: Object.values(completedActions).reduce((sum, count) => sum + count, 0),
...
function generateNarrativeSummary(completedSummary, activeSummary, upcomingCount, topIdeasCount, totalEvents) {
    const parts = [];
