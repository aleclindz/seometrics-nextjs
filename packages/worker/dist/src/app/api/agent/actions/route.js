    ...
    try {
      ...
      `)
        ...
        if (error) {
            ...
            return server_1.NextResponse.json({ error: 'Failed to fetch actions' }, { status: 500 });
        }
        // Get summary stats
        const { data: stats, error: statsError } = await supabase
            ...
            .select('status')
            .eq('user_token', userToken);
        const statusCounts = stats?.reduce((acc, action) => {
...
async function POST(request) {
    try {
        ...
        const { userToken, siteUrl, ideaId, actionType, title, description, payload, policy, priorityScore, scheduledFor } = body;
        if (!userToken || !siteUrl || !actionType || !title) {
            return server_1.NextResponse.json({ error: 'Missing required fields: userToken, siteUrl, actionType, title' }, { status: 400 });
        }
        ...
        if (ideaId) {
            const { data: idea, error: ideaError } = await supabase
                ...
                .single();
            if (ideaError || !idea) {
                return server_1.NextResponse.json({ error: 'Invalid idea reference' }, { status: 400 });
            }
        ...
        if (error) {
            ...
            return server_1.NextResponse.json({ error: 'Failed to create action' }, { status: 500 });
        }
...
async function PUT(request) {
    try {
        ...
        if (!actionId || !userToken) {
            return server_1.NextResponse.json({ error: 'Missing required fields: actionId, userToken' }, { status: 400 });
        }
        // Verify ownership
        ...
        if (fetchError || !existingAction) {
            return server_1.NextResponse.json({ error: 'Action not found' }, { status: 404 });
        }
        // Prepare update data
        ...
        if (updateError) {
            ...
            return server_1.NextResponse.json({ error: 'Failed to update action' }, { status: 500 });
        }
        // Queue for execution if requested
        if (queueForExecution && updatedAction.status === 'queued') {
            try {
                const jobId = await queue_manager_1.queueManager.queueAction(actionId, userToken, updatedAction.action_type, updatedAction.payload, updatedAction.policy, {
