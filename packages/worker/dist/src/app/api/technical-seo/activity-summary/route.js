...
const activity_aggregator_1 = require("@/lib/activity-aggregator");
...
async function POST(request) {
    try {
        ...
        const { userToken, siteUrl, forceRefresh = false, sinceDays = 7, websiteStatus } = await request.json();
        ...
        if (!forceRefresh) {
            const cachedSummary = await activity_aggregator_1.ActivityAggregator.getCachedSummary(userToken, siteUrl);
            if (cachedSummary) {
        ...
        const aggregator = new activity_aggregator_1.ActivityAggregator(userToken, siteUrl, sinceDays);
        ...
        // Cache the summary
        await activity_aggregator_1.ActivityAggregator.saveSummary(userToken, siteUrl, aiSummary, aggregatedActivity, userData.id);
        ...
        return server_1.NextResponse.json({ error: 'Failed to generate activity summary' }, { status: 500 });
    }
}
async function generateAISummary(siteUrl, activity, userToken, websiteStatus) {
    ...
    if (userToken) {
        try {
            ...
            if (technicalStatus && technicalStatus.issues && technicalStatus.issues.length > 0) {
                ...
                const specificIssues = issues.slice(0, 3).map((issue) => {
...
function generateNextSteps(websiteStatus, domainName) {
    if (!websiteStatus) {
...
function generateFallbackSummary(domainName, timePeriod, activity) {
...
async function summarizeActivitiesByTypeWithMetrics(activities, userToken) {
...
async function generateWelcomeMessage(websiteStatus, domainName, userToken, siteUrl) {
...
async function enhanceWebsiteStatus(websiteStatus, userToken, siteUrl, aggregatedActivity) {
    try {
        ...
        try {
            ...
            enhanced.totalPages = Math.max(enhanced.totalPages || 0, (metaTags?.length || 0) + (altTags?.length || 0));
...
async function fetchTechnicalSEOStatus(userToken, siteUrl) {
    try {
        ...
        if (result.success && result.actionItems) {
            ...
            return {
                ...
                summary: {
                    ...
                    autoFixable: items.filter((i) => ['sitemap_missing', 'sitemap_not_downloaded', 'sitemap_not_submitted', 'sitemap_broken', 'sitemap_dynamic_not_submitted', 'robots_not_analyzed', 'schema_missing_pages', 'indexing_blocked_pages'].includes(i.issue_type)).length
                }
            };
        }
        return {
