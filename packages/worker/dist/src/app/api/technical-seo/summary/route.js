    ...
    try {
        ...
        try {
            ...
            if (inspectionsError && inspectionsError.code !== '42P01') {
                ...
                return server_1.NextResponse.json({ error: 'Failed to fetch inspection data' }, { status: 500 });
            }
        }
        catch (error) {
            if (error?.code === '42P01') {
        ...
        if (totalPages === 0 && schemaActivity?.length) {
            ...
            const uniquePages = new Set(schemaActivity
            ...
            totalPages = uniquePages.size;
            withSchema = schemaActivity.filter(activity => activity.event_type === 'schema_generated' ||
                (activity.metadata?.schemas_added && activity.metadata.schemas_added > 0)).length;
        ...
        if (recentActivity?.length) {
            const failedEvents = recentActivity.filter(event => event.new_state === 'failed' ||
        ...
        if (!processedIssues.length && inspections) {
            const canonicalIssues = inspections.filter(i => i.user_canonical && i.google_canonical && i.user_canonical !== i.google_canonical).length;
        ...
        if (robotsActivity?.length) {
            const robotsIssues = robotsActivity.filter(event => event.event_type.includes('error') ||
                event.metadata?.issues_found);
            robotsIssues.forEach((robotsEvent) => {
                ...
                issues.forEach((issue) => {
                    processedIssues.push({
                        type: `Robots.txt: ${issue.type.replace(/_/g, ' ').replace(/\b\w/g, (l) => l.toUpperCase())}`,
        ...
        if (recentActivity?.length) {
            automatedFixes = recentActivity.filter(event => ['schema_generated', 'meta_tags_updated', 'technical_fix_applied', 'sitemap_generated'].includes(event.event_type) &&
                event.new_state !== 'failed').length;
            pendingFixes = recentActivity.filter(event => event.new_state === 'pending' || event.new_state === 'in_progress').length;
            fixErrors = recentActivity.filter(event => event.new_state === 'failed' || event.event_type.includes('error')).length;
        }
        else {
        ...
        return server_1.NextResponse.json({ error: 'Failed to fetch technical SEO summary' }, { status: 500 });
    }
}
