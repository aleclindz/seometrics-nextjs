...
async function POST(request) {
    try {
        ...
        return server_1.NextResponse.json({ error: 'SEO Watchdog testing failed' }, { status: 500 });
    }
}
async function testDatabaseSchema(userToken) {
    const tests = [];
...
async function testAPIEndpoints(userToken, siteUrl) {
    const tests = [];
...
async function testSEOAgentJS(siteUrl) {
    const tests = [];
    ...
    try {
        ...
        if (response.ok) {
            ...
            const presentFeatures = enhancedFeatures.filter(feature => jsContent.toLowerCase().includes(feature.toLowerCase()));
            tests.push({
...
async function testMonitoringCapabilities(userToken, siteUrl) {
    const tests = [];
...
async function testFixAutomation(userToken, siteUrl) {
    const tests = [];
...
async function logTestResults(userToken, testResults) {
    try {
...
async function GET(request) {
    try {
        ...
        console.error('[TEST-SEO-WATCHDOG] Error:', error);
        return server_1.NextResponse.json({ error: 'Failed to get test capabilities' }, { status: 500 });
    }
}
