...
async function POST(request) {
    try {
        ...
        return server_1.NextResponse.json({ error: 'Hreflang fix failed' }, { status: 500 });
    }
}
async function analyzeLanguageStructure(siteUrl) {
    const fixes = [];
    const errors = [];
...
async function applyHreflangAutomation(siteUrl, pageUrl) {
    const fixes = [];
    const errors = [];
...
async function generateHreflangActionItems(userToken, siteUrl, fixResults) {
    try {
        // Only create action items for manual fixes that need attention
        const manualFixes = fixResults.fixes.filter((fix) => !fix.automated && ['critical', 'warning'].includes(fix.priority));
...
async function logHreflangFixesToEvents(userToken, fixResults) {
    try {
...
async function GET(request) {
    try {
        ...
        console.error('[FIX-HREFLANG] Error:', error);
        return server_1.NextResponse.json({ error: 'Failed to get hreflang capabilities' }, { status: 500 });
    }
}
