...
async function POST(request) {
    try {
        ...
        return server_1.NextResponse.json({ error: 'Indexability fix failed' }, { status: 500 });
    }
}
async function fixNoindexIssues(siteUrl, userToken) {
    const fixes = [];
    const errors = [];
    try {
...
async function suggestRobotsFixes(siteUrl, userToken) {
    const fixes = [];
    const errors = [];
    try {
        ...
        if (response.ok) {
            ...
            if (robotsData.issues && robotsData.issues.length > 0) {
                robotsData.issues.forEach((issue) => {
                    fixes.push({
...
async function fixCanonicalIssues(siteUrl) {
    const fixes = [];
    const errors = [];
...
async function generateActionItems(userToken, siteUrl, fixResults) {
    try {
        const manualFixes = fixResults.fixes.filter((fix) => !fix.automated && fix.priority === 'critical');
...
async function logFixesToEvents(userToken, fixResults) {
    try {
...
async function GET(request) {
    try {
        ...
        console.error('[FIX-INDEXABILITY] Error:', error);
        return server_1.NextResponse.json({ error: 'Failed to get available fixes' }, { status: 500 });
    }
}
