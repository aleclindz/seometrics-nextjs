...
async function POST(request) {
    try {
        const { websiteUrl, websiteToken, userToken } = await request.json();
        if (!websiteUrl) {
        ...
        if (!websiteContent) {
            return server_1.NextResponse.json({ error: 'Could not fetch website content' }, { status: 400 });
        }
...
function analyzeBusinessSignals(content, patterns) {
...
function findKeywordMatches(content, keywords) {
    if (!keywords)
        return [];
    const matches = [];
...
function findRegexMatches(content, regex) {
    const matches = [];
    let match;
...
function extractBusinessInfo(patternType, matches, businessInfo) {
    switch (patternType) {
        case 'contact':
...
function determineBusinessType(signals, confidence) {
    if (confidence < 20) {
        return 'unknown';
    }
...
async function getRecommendedSchemaType(signals) {
    try {
...
async function storeDetectionResults(websiteToken, userToken, result) {
    try {
        await supabase
            .from('websites')
            .update({
