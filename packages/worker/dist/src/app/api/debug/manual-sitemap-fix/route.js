...
async function POST(request) {
    try {
        ...
        let websiteRecord;
        const websiteResult = await DomainQueryService_1.DomainQueryService.findWebsiteByDomain(userToken, targetDomain);
        if (websiteResult.success && websiteResult.data) {
            ...
            const createResult = await DomainQueryService_1.DomainQueryService.createWebsiteWithDomain(userToken, targetDomain, {
            ...
            if (!createResult.success) {
                return server_1.NextResponse.json({ error: 'Failed to create website', details: createResult.error }, { status: 500 });
            }
    ...
    <priority>0.3</priority>
        ...
        if (existingSitemap) {
            ...
            if (updateError) {
                ...
                return server_1.NextResponse.json({ error: 'Failed to update sitemap', details: updateError.message }, { status: 500 });
            }
            ...
            if (insertError) {
                ...
                return server_1.NextResponse.json({ error: 'Failed to store sitemap', details: insertError.message }, { status: 500 });
            }
