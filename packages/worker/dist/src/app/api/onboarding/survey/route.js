...
async function POST(request) {
    try {
        ...
        const { userToken, websiteBuildingMethod, websiteBuildingMethodOther, usesCms, cmsType, cmsTypeOther, hostingProvider, hostingProviderOther, businessType, websiteAge, monthlyVisitors, seoExperience, primarySeoGoal, interestedInFounderCall, acceptedProOffer } = body;
        if (!userToken) {
        ...
        if (!websiteBuildingMethod || !hostingProvider || !businessType || !websiteAge || !monthlyVisitors || !seoExperience) {
            return server_1.NextResponse.json({ error: 'Missing required survey fields' }, { status: 400 });
        }
        ...
        const proOfferRedemptionCode = acceptedProOffer
            ? `FOUNDER_${crypto_1.default.randomBytes(8).toString('hex').toUpperCase()}`
            : null;
        ...
        if (checkError && checkError.code !== 'PGRST116') { // PGRST116 = not found
            ...
            if (checkError.message?.includes('relation "onboarding_surveys" does not exist') ||
                ...
                return server_1.NextResponse.json({ error: 'Onboarding system not yet configured. Please contact support.' }, { status: 503 } // Service unavailable
                );
            }
            return server_1.NextResponse.json({ error: 'Database error checking existing survey' }, { status: 500 });
        }
        let surveyResult;
        if (existingSurvey) {
            ...
            if (error) {
                ...
                return server_1.NextResponse.json({ error: 'Failed to update survey' }, { status: 500 });
            }
            surveyResult = data;
        }
        else {
            ...
            if (error) {
                ...
                return server_1.NextResponse.json({ error: 'Failed to create survey' }, { status: 500 });
            }
            surveyResult = data;
        }
    ...
    try {
        ...
        if (error) {
            if (error.code === 'PGRST116') {
                // Survey not found
                return server_1.NextResponse.json({ error: 'Survey not found' }, { status: 404 });
            }
            // Check if table doesn't exist
            if (error.message?.includes('relation "onboarding_surveys" does not exist') ||
                error.code === '42P01') {
                console.error('[ONBOARDING] onboarding_surveys table does not exist - migration needed');
                return server_1.NextResponse.json({ error: 'Survey not found' }, { status: 404 } // Return 404 so onboarding still triggers
                );
            }
            console.error('[ONBOARDING] Error fetching survey:', error);
            return server_1.NextResponse.json({ error: 'Failed to fetch survey' }, { status: 500 });
        }
        return server_1.NextResponse.json({
            success: true,
            survey
