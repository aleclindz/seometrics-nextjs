...
async function GET(request) {
    try {
        ...
        return server_1.NextResponse.json({ error: 'Internal server error during OAuth flow' }, { status: 500 });
    }
}
/**
 * Initiate Vercel OAuth flow
 */
async function initiateOAuthFlow(userToken) {
    try {
        ...
        if (isMarketplaceFlow) {
            ...
            return server_1.NextResponse.redirect(oauthUrl);
        }
        else {
...
async function handleOAuthCallback(code, state) {
    try {
        ...
        if (stateError || !oauthState) {
            ...
            return server_1.NextResponse.json({ error: 'Invalid or expired OAuth state' }, { status: 400 });
        }
        ...
        if (!process.env.VERCEL_CLIENT_ID || !process.env.VERCEL_CLIENT_SECRET ||
            ...
            return server_1.NextResponse.json({ error: 'Vercel OAuth credentials not configured. Please set up your Vercel Integration first.' }, { status: 500 });
        }
        ...
        if (!tokenResponse.ok) {
            ...
            return server_1.NextResponse.json({ error: 'Failed to exchange authorization code for access token' }, { status: 400 });
        }
        const tokenData = await tokenResponse.json();
        if (!tokenData.access_token) {
            console.error('[VERCEL OAUTH] No access token in response:', tokenData);
            return server_1.NextResponse.json({ error: 'No access token received from Vercel' }, { status: 400 });
        }
        ...
        if (!userResponse.ok) {
            ...
            return server_1.NextResponse.json({ error: 'Failed to get user information from Vercel' }, { status: 400 });
        }
        ...
        if (isAnonymousFlow) {
            ...
            return server_1.NextResponse.redirect(signupUrl);
        }
        // Clean up OAuth state
        await supabase
            .from('oauth_states')
        ...
        return server_1.NextResponse.redirect(successUrl);
    }
    catch (error) {
        ...
        return server_1.NextResponse.redirect(errorUrl);
    }
}
/**
...
async function POST(request) {
    try {
        const { userToken, accessToken, teamId, projectId, projectName, deploymentMethod = 'redirects', autoDeployment = false } = await request.json();
        if (!userToken || !accessToken || !projectId) {
            return server_1.NextResponse.json({ error: 'Missing required parameters: userToken, accessToken, projectId' }, { status: 400 });
        }
        ...
        console.error('[VERCEL OAUTH] Error creating integration:', error);
        return server_1.NextResponse.json({ error: 'Internal server error creating integration' }, { status: 500 });
    }
}
