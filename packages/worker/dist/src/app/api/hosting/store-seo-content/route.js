...
async function POST(request) {
    try {
        ...
        if (!domain || !userToken) {
            return server_1.NextResponse.json({ error: 'Missing required parameters: domain and userToken' }, { status: 400 });
        }
        if (!sitemapContent && !robotsContent) {
            return server_1.NextResponse.json({ error: 'At least one of sitemapContent or robotsContent is required' }, { status: 400 });
        }
        ...
        if (error) {
            ...
            return server_1.NextResponse.json({ error: 'Failed to store SEO content' }, { status: 500 });
        }
        const response = {
            success: true,
        ...
        return server_1.NextResponse.json({ error: 'Internal server error storing SEO content' }, { status: 500 });
    }
}
async function GET(request) {
    try {
        ...
        if (error) {
            ...
            return server_1.NextResponse.json({ error: 'SEO content not found' }, { status: 404 });
        }
        ...
        return server_1.NextResponse.json({ error: 'Internal server error retrieving SEO content' }, { status: 500 });
    }
}
async function DELETE(request) {
    try {
        ...
        if (error) {
            ...
            return server_1.NextResponse.json({ error: 'Failed to delete SEO content' }, { status: 500 });
        }
        return server_1.NextResponse.json({
            success: true,
        ...
        return server_1.NextResponse.json({ error: 'Internal server error deleting SEO content' }, { status: 500 });
    }
}
