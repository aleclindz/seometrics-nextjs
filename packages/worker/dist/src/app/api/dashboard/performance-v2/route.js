...
async function GET(request) {
    try {
        ...
        dailyComparisonResult, topQueriesResult, topPagesResult, deviceBreakdownResult, countryBreakdownResult, appearanceBreakdownResult] = await Promise.all([
            dailyMetricsQuery,
        ...
        const errors = [
            ...
            dailyComparisonResult && dailyComparisonResult.error,
        ...
        if (errors.length > 0) {
            ...
            return server_1.NextResponse.json({ error: 'Database query failed' }, { status: 500 });
        }
        ...
        const comparisonDaily = dailyComparisonResult?.data || [];
        const comparisonTotals = comparisonDaily.reduce((acc, d) => ({
            clicks: acc.clicks + (d.clicks || 0),
            impressions: acc.impressions + (d.impressions || 0),
            ctr: acc.ctr + (d.ctr || 0),
            position: acc.position + (d.position || 0)
        }), { clicks: 0, impressions: 0, ctr: 0, position: 0 });
        if (comparisonDaily.length > 0) {
            comparisonTotals.position = comparisonTotals.position / comparisonDaily.length;
        ...
        let topQueriesData = topQueriesResult.data || [];
        if (!topQueriesData || topQueriesData.length === 0) {
            ...
            if (!fallback.error && fallback.data)
                topQueriesData = fallback.data;
        }
        let topPagesData = topPagesResult.data || [];
        ...
        const performanceData = {
            ...
            topQueries: (topQueriesData || []).slice(0, 10).map((q) => ({
                query: q.query,
                clicks: q.clicks || 0,
            ...
            topPages: (topPagesData || []).slice(0, 10).map((p) => ({
                page: p.page,
                clicks: p.clicks || 0,
 ...
 */
function calculateTrend(current, previous, lowerIsBetter = false) {
