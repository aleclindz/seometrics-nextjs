...
async function GET(request) {
    ...
    try {
        ...
        if (!apiKey)
            results.errors.push('OpenAI API key not configured');
        ...
        if (apiKey) {
            try {
                ...
                const response = await openaiClient.sendMessage("Just say 'Health check successful' in one sentence.", testContext);
                ...
                if (!response.content)
                    results.errors.push('Agent not responding to basic queries');
            }
            catch (agentError) {
                results.checks.agent_response = {
                    status: 'fail',
        ...
        results.responseTime = Date.now() - startTime;
        return server_1.NextResponse.json(results, { status: 500 });
    }
}
async function POST(request) {
    ...
    if (!userToken) {
        return server_1.NextResponse.json({ error: 'User token required for extended tests' }, { status: 400 });
    }
    const results = {
        timestamp: new Date().toISOString(),
        status: 'healthy',
    ...
    const apiKey = process.env.OPENAI_API_KEY;
    if (!apiKey) {
        return server_1.NextResponse.json({ error: 'OpenAI API key not configured' }, { status: 500 });
    }
    try {
        const openaiClient = new openai_function_client_1.OpenAIFunctionClient(apiKey);
        const testContext = {
