...
async function GET(request) {
    try {
        ...
        if (!userToken) {
            return server_1.NextResponse.json({ error: 'User token is required' }, { status: 401 });
        }
    ...
    try {
        ...
        try {
            ...
            const { data: existingContent } = await supabase
                ...
                .eq('website_token', targetWebsiteToken);
            const combined = [];
            (existingKw || []).forEach((k) => { if (k?.topic_cluster)
                combined.push(k.topic_cluster); });
            (existingContent || []).forEach((c) => { if (c?.topic_cluster)
                combined.push(c.topic_cluster); });
            const seen = {};
            const unique = [];
...
async function DELETE(request) {
    try {
        ...
        if (!userToken || !websiteToken) {
            return server_1.NextResponse.json({ error: 'User token and website token are required' }, { status: 400 });
        }
        // Verify user owns this website
        const { data: website, error: websiteError } = await supabase
            .from('websites')
        ...
        if (keyword) {
            ...
            if (deleteError) {
                ...
                return server_1.NextResponse.json({ error: 'Failed to delete keyword' }, { status: 500 });
            }
            return server_1.NextResponse.json({ success: true, message: 'Keyword deleted successfully' });
        }
        if (topicCluster) {
            ...
            if (deleteKeywordsError || deleteContentError) {
                ...
                return server_1.NextResponse.json({ error: 'Failed to delete topic cluster' }, { status: 500 });
            }
            return server_1.NextResponse.json({ success: true, message: 'Topic cluster deleted successfully' });
        }
        return server_1.NextResponse.json({ error: 'Either keyword or topicCluster parameter is required' }, { status: 400 });
    }
    catch (error) {
        console.error('[KEYWORD STRATEGY] Unexpected error:', error);
        return server_1.NextResponse.json({ error: 'Internal server error' }, { status: 500 });
...
async function getWebsiteKeywordStrategy(userToken, websiteToken, returnResponse = true) {
    try {
        ...
        if (keywordsError) {
            ...
            if (returnResponse) {
                return server_1.NextResponse.json({ error: 'Failed to fetch keywords' }, { status: 500 });
            }
            return null;
        }
        ...
        if (clusterError) {
            ...
            if (returnResponse) {
                return server_1.NextResponse.json({ error: 'Failed to fetch cluster content' }, { status: 500 });
            }
            return null;
        }
