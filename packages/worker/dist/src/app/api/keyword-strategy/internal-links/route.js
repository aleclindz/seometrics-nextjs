...
async function POST(request) {
    try {
        ...
        const { userToken, websiteToken, domain, sourceArticleId, targetArticleId, targetUrl, anchorText, topicCluster, linkContext } = body;
        if (!userToken) {
            return server_1.NextResponse.json({ error: 'User token is required' }, { status: 401 });
        }
        // Find website token if domain provided
        ...
        if (linkError) {
            ...
            return server_1.NextResponse.json({ error: 'Failed to create internal link' }, { status: 500 });
        }
        return server_1.NextResponse.json({
            success: true,
...
async function DELETE(request) {
    try {
        ...
        if (!userToken || !linkId) {
            return server_1.NextResponse.json({ error: 'User token and link ID are required' }, { status: 400 });
        }
        ...
        const { data: link, error: linkError } = await supabase
            ...
            .single();
        if (linkError || !link) {
            return server_1.NextResponse.json({ error: 'Internal link not found' }, { status: 404 });
        }
        ...
        if (deleteError) {
            ...
            return server_1.NextResponse.json({ error: 'Failed to delete internal link' }, { status: 500 });
        }
        return server_1.NextResponse.json({
            success: true,
...
async function getInternalLinkOpportunities(websiteToken, topicCluster) {
    try {
        let query = supabase
        ...
        if (error) {
            ...
            return server_1.NextResponse.json({ error: 'Failed to fetch link opportunities' }, { status: 500 });
        }
...
async function getExistingInternalLinks(websiteToken, topicCluster) {
    try {
        let query = supabase
            .from('internal_links')
            .select(`
      ...
      `)
        ...
        if (error) {
            ...
            return server_1.NextResponse.json({ error: 'Failed to fetch existing links' }, { status: 500 });
        }
        // Get statistics
