...
function heuristicGenerateKeywords(opts) {
    ...
    while (out.length < Math.max(5, Math.min(opts.generateCount, 30)) && i < 200) {
        ...
        if (out.some(k => k.keyword.toLowerCase() === kw.toLowerCase()))
            continue;
        out.push({
            keyword: kw,
...
async function POST(request) {
    try {
        ...
        const { userToken, websiteToken, domain, baseKeywords = [], topicFocus = '', generateCount = 10, avoidDuplicates = true } = body;
        if (!userToken) {
...
Return only the JSON array, no other text.`;
        ...
        if (avoidDuplicates && existingKeywords.length > 0) {
            generatedKeywords = generatedKeywords.filter((kw) => !existingKeywords.includes(kw.keyword.toLowerCase()));
        }
...
async function GET(request) {
    try {
        ...
        const uniqueClusters = clusters
            ? Array.from(new Set(clusters.map((c) => c.topic_cluster)))
            : [];
        return server_1.NextResponse.json({
            success: true,
